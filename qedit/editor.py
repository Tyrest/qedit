...
def generate_setpts_filter(sections: list):
    """This method generates a setpts filter like the following
    'if(between(t,1.17,1.29),PTS-STARTPTS,if(between(t,1.292496,1.65),PTS-STARTPTS+1.292496, ...))

    Args:
        sections (list): Tuples of content start and end timestamps.
    """
    filter = ""
    for i, section in enumerate(sections):
        if i == 0:
            filter += f"if(between(t,{section[0]},{section[1]}),PTS-STARTPTS"
        else:
            filter += f",if(between(t,{section[0]},{section[1]}),PTS-STARTPTS+{section[0]}"
    filter += "".join([")" for _ in range(len(sections))])
    return filter
...
def generate_ffmpeg_commands(video: str, output: str, sections: list):
    ...
    for i, section_batch in enumerate(section_batches):
        ...
        for section, factor in section_batch:
            # if factor == 1:
            #     filters.append(v.trim(start=section[0], end=section[1]))
            #     filters.append(a.filter("atrim", start=section[0], end=section[1]))
            # else:
            filters.append(
        ...
        # filters.append(v.filter("setpts", generate_setpts_filter(section_batch)))
        # filters.append(a.filter("asetpts", generate_setpts_filter(section_batch)))
...
def monitor_progress_of_multiple_ffmpeg_processes(duration, processes, prompt):
    with Progress() as progress:
        task = progress.add_task(
            prompt,
...
def monitor_progress_of_ffmpeg_process(duration, process, prompt):
    with Progress() as progress:
        task = progress.add_task(
            prompt,
            total=duration * 1_000_000,
...
def run_ffmpeg(video: str, output: str, sections: list):
    ...
    print(f"First command: {commands[0].compile()}")
    ...
    try:
        ...
        monitor_progress_of_multiple_ffmpeg_processes(duration, processes, "Cutting...")
        ...
        with concat.run_async(pipe_stdout=True) as process:
            monitor_progress_of_ffmpeg_process(duration, process, "Concatenating...")
